...
class GlobalAdminNumberCheck(AbstractCisCheck):
    ...
    def check_compliance(self, session, graph_explorer_headers):
        ...
        if(2 <= n_of_admins <= 4):
            ...
            return True
        print("--> [X] In the organization there isn't the recommended number of global admins")    
        return False

    def __get_global_admin_role_id(session, graph_explorer_headers):
        print("[*] Retrieving the directory roles...")
        res = session.get(ROLES_LINK, headers = graph_explorer_headers)
        # Check the response status code
        if(res.status_code == 200):
            print("[*] The directory roles has been correctly obtained")
            ...
            for role in roles["value"]:
                if role["displayName"] == "Global Administrator":
                    print("[*] Found the Global Administrator role id")
                    return role["id"]
            print("--> [X] Could not find the global administrator role among the roles obtained")
            exit(1)
        else:
            print("--> [X] A problem was encountered while sending the directory roles request")
            print(res.text)
            exit(1)    
    
    def __get_global_admins(session, graph_explorer_headers, global_admin_role_id):
